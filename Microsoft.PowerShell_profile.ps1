...
# 0. Resolve true repository root (handles symlinks on macOS & Windows)
...
if ($PSCommandPath -and (Test-Path $PSCommandPath)) {
    ...
    if ($item -and $item.LinkType) {
        $target = $item.Target
        if ([array]$target) { $target = $target[0] }
        if (Test-Path $target) {
...
# Fallback candidate check if Modules directory is not found in PSScriptRoot
